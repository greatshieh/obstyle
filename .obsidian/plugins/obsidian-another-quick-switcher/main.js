...
var AppHelper = class {
  ...
  captureState() {
    const existing = /* @__PURE__ */ new WeakSet();
    const oldLeaf = this.unsafeApp.workspace.activeLeaf;
    app.workspace.iterateAllLeaves((leaf2) => existing.add(leaf2));
    let leaf = app.workspace.getLeaf(), state = leaf.getViewState(), eState = leaf.getEphemeralState();
    return {
      leaf,
      async restore() {
        if (!leaf)
        ...
        if (existing.has(leaf)) {
          await leaf.setViewState({ ...state, active: leaf === oldLeaf, popstate: true }, eState);
          if (oldLeaf && leaf !== oldLeaf)
            app.workspace.setActiveLeaf(oldLeaf, { focus: true });
        } else {
          leaf.detach();
        }
        leaf = this.leaf = void 0;
      }
    };
  }
...
var AnotherQuickSwitcherModal = class extends import_obsidian4.SuggestModal {
  ...
  async chooseCurrentSuggestion(leaf, option = {}) {
    ...
    if (!option.keepOpen) {
      ...
      (_d = this.initialState) != null ? _d : this.initialState = this.appHelper.captureState();
    }
...
var GrepModal = class extends import_obsidian7.SuggestModal {
  ...
  async chooseCurrentSuggestion(leaf, option = {}) {
    ...
    if (!option.keepOpen) {
      ...
      (_b = this.initialState) != null ? _b : this.initialState = this.appHelper.captureState();
    }
